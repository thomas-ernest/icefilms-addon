...
def handle_captchas(url, html, data, dialog):
    ...
    if solvemedia:
        ...
        if alt_frame:
            ...
            open(puzzle_img, 'wb').write(net.http_GET(
                "http://api.solvemedia.com%s" % re.search('<img src="(/papi/media[^"]+)"', html).group(1)).content)
...
def resolve_uploadx(url):
    try:
        ...
        free_download_form = re.findall('''(<form [^>]*action=["']{2}[^>]*>.*?</form>)''', html,
                                        re.DOTALL | re.IGNORECASE).pop()
...
def SHARED2_HANDLER(url):
    ...
    if re.search('Your free download limit is over.', html):
        ...
        Notify('big', '2Shared Download Limit Exceeded', 'You have reached your download limit', '', '',
               'You must wait ' + wait_time + ' to try again')
        return None
...
def resolve_tusfiles(url):
    try:
        ...
        filename = re.search('Start download<h1><span class="label label-default"><FONT COLOR="#ffffff">(.+?)</FONT>',
                             html).group(1)
...
def resolve_xfileload(url):
    try:
        ...
        # Get download link
        dialog.update(100)
        link = re.search('<a href="(.+?)" target=""><img src="http://xfileload.com/3ghdes/images/downdown.png" /></a>',
                         html)
        if link:
            return link.group(1)
        else:
