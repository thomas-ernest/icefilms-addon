...
try:
    ...
    xbmcgui.Dialog().ok("Icefilms Import Failure", "Failed to import addon.common",
                        "A component needed by Icefilms is missing on your system",
                        "Please visit www.tvaddons.ag for support")
net = Net()
...
try:
    ...
    xbmcgui.Dialog().ok("Icefilms Import Failure", "Failed to import Metahandlers",
                        "A component needed by Icefilms is missing on your system",
                        "Please visit www.tvaddons.ag for support")
...
def Startup_Routines():
    ...
    if next_aired:
        xbmc.executebuiltin(
            "RunScript(%s, silent=true)" % os.path.join(icepath, 'resources/script.tv.show.next.aired/default.py'))
    ...
    if debrid_account:
        if not addon.get_setting('realdebrid_token'):
            ...
            try:
                ...
                Notify('big', 'Real-Debrid Login Failed', 'Failed to connect with Real-Debrid.', '', '',
...
def ContainerStartup():
    ...
    if not meta_installed:
        ...
        ret = dialog.yesno('Download Meta Containers ' + str(containers['date']) + ' ?',
                           'There is a metadata container avaliable.', 'Install it to get meta information for videos.',
                           'Would you like to get it? Its a small ' + str(containers['db_size']) + 'MB download.',
...
def create_meta_pack():
    ...
    mh = metahandlers.MetaData()
    mh.insert_meta_installed(addon_id, last_update='Now', movie_covers='true', tv_covers='true', tv_banners='true',
                             movie_backdrops='true', tv_backdrops='true')
    ...
    mh.update_meta_installed(addon_id, movie_covers='false', tv_covers='false', tv_banners='false',
                             movie_backdrops='false', tv_backdrops='false')
...
def ADD_TO_FAVOURITES(name, url, imdbnum, videoType):
    ...
    try:
        if name and url:
            ...
            next_aired = str2bool(addon.get_setting('next-aired'))
            if next_aired:
                xbmc.executebuiltin("RunScript(%s, silent=true)" % os.path.join(icepath,
                                                                                'resources/script.tv.show.next.aired/default.py'))
        ...
        if meta_setting == 'true' and meta_installed:
            ...
            if fav[2] is not None:
                ...
                if meta is None:
                    ...
                    addDir(fav[0], new_url, mode, '', meta=meta, delfromfav=True, imdb=fav[2],
                           totalItems=len(new_fav_list), meta_install=meta_installed, favourite=True)
            else:
                # add all the items without meta
                addDir(fav[0], new_url, mode, '', delfromfav=True, totalItems=len(new_fav_list), favourite=True)
        else:
    ...
    if recent_movies:
        ...
        for url, name, hd in mirlinks:
            ...
            if meta_installed and meta_setting == 'true':
                ...
                addDir(new_name, url, 100, '', meta=meta, disablefav=True, disablewatch=True,
                       meta_install=meta_installed)
            else:
    ...
    for scrape in homepage:
        ...
        for scraped in latrel:
            ...
            for url, name, hd in mirlinks:
                ...
                if meta_installed and meta_setting == 'true':
                    meta = check_video_meta(name, metaget)
                    addDir(new_name, url, mode, '', meta=meta, disablefav=True, disablewatch=True,
                           meta_install=meta_installed)
                else:
        ...
        if meta_setting == 'true' and meta_installed:
            ...
            if watch[4] is not None:
                ...
                if meta is None:
                    ...
                    # add directories with meta
                    addDir(new_name, new_url, mode, '', meta=meta, imdb=watch[4], totalItems=len(watch_list),
                           meta_install=meta_installed, recentWatched=True)
            else:
                # add all the items without meta
                addDir(new_name, new_url, mode, '', totalItems=len(watch_list), recentWatched=True)
        else:
...
def clear_watched(videoType=None):
    dialog = xbmcgui.Dialog()
    if videoType:
        ret = dialog.yesno('Delete Watched List?', 'Do you wish to delete the current watched list?', '',
                           'This cannot be undone!')
    else:
        ret = dialog.yesno('Delete Watched Lists?', 'Do you wish to delete all of your watched lists?', '',
                           'This cannot be undone!')
...
def clear_queue(videoType=None):
    dialog = xbmcgui.Dialog()
    if videoType:
        ret = dialog.yesno('Delete Queue List?', 'Do you wish to delete the current queue list?', '',
                           'This cannot be undone!')
    else:
        ret = dialog.yesno('Delete Queue Lists?', 'Do you wish to delete all of your queue lists?', '',
                           'This cannot be undone!')
        ...
        if meta_setting == 'true' and meta_installed:
            ...
            if queue[4] is not None:
                ...
                if meta is None:
                    ...
                    # add directories with meta
                    addDir(new_name, new_url, mode, '', meta=meta, imdb=queue[4], totalItems=len(queue_list),
                           meta_install=meta_installed, queueList=True)
            else:
                # add all the items without meta
                addDir(new_name, new_url, mode, '', totalItems=len(queue_list), queueList=True)
        else:
...
def add_queue():
    try:
        ...
        db_connection.save_queue(parse_url(url), video_type, video['name'], video['year'], season_num, episode_num,
                                 imdbnum)
...
def DoSearch(iurl, search, nextPage):
    ...
    if more:
        ...
        u = sys.argv[0] + "?url=" + sysurl + "&mode=" + str(
            555) + "&name=" + sysname + "&search=" + search + "&nextPage=" + str(nextPage)
...
def MOVIEINDEX(url):
    ...
    temp = re.compile(
        '(<h3>|<a class=imdb id=.+?></a><a class=tube></a><i class=star></i><a href=)(.+?)(<div|</h3>|>(.+?)<br>)').findall(
        link)
    for tag, link, longname, name in temp:
        ...
        if tag == '<h3>':
            ...
            scrape = re.compile(
                '<a class=imdb id=(.+?)></a><a class=tube></a><i class=star></i><a href=/(.+?)>(.+?)<br>').findall(
                string)
...
def TVINDEX(url):
    ...
    for entry in temp:
        ...
        if text:
            folder_tags('[COLOR blue]' + text[0] + '[/COLOR]')
        scrape = re.compile(
            '<a class=imdb id=(.+?)></a><a class=tube></a><i class=star></i><a href=/(.+?)>(.+?)</a>').findall(entry)
        if scrape:
            for imdb_id, url, name in scrape:
                if meta_setting == 'true':
        ...
        if FlattenSingleSeasons == True and listlength <= 1:
            ...
            if meta_installed and meta_setting == 'true' and season_meta:
                ...
                addDir(seasons.strip(), '', 13, temp['cover_url'], imdb='' + str(imdb_id), meta=season_meta[num],
                       totalItems=len(season_list), meta_install=meta_installed)
                num = num + 1
            else:
...
def LOADMIRRORS(url):
    ...
    if not namematch:
        Notify('big', 'Error Loading Sources',
               'An error occured loading sources.\nCheck your connection and/or the Icefilms site.', '')
    ...
    if epcheck1 or epcheck2:
        ...
        try:
            ...
            if cache.get('mediatvshowname'):
                ...
                cache.set('mediapath',
                          'TV Shows/' + Clean_Windows_String(showname) + '/' + Clean_Windows_String(seasonname))
            else:
...
def PART(scrap, sourcenumber, host, args, source_tag, ice_meta=None, video_url=None, debrid_hosts=None):
    ...
    if checkforsource:
        ...
        # if source does not have multiple parts...
            ...
            source5 = re.compile(
                '<a\s+rel=' + sourcenumber + '.+?onclick=\'go\((\d+)\)\'>Source\s+#' + sourcenumber + ':').findall(
                scrap)
...
def GetURL(url, params=None, referrer=ICEFILMS_REFERRER, use_cookie=False, save_cookie=False, use_cache=True):
    ...
    try:
        ...
        Notify('big', 'Error Requesting Site', 'An error has occured communicating with Icefilms', '', '',
               'Check your internet connection and the Icefilms site.')
        html = ''
        pass
...
def Item_Meta(name, resume_point=0):
    ...
    try:
        open_vidname = cache.get('videoname')
    except:
        vidname = ''
        addon.log_error('OPENING VIDNAME FAILED!')
    else:
        try:
            get_vidname = htmlcleaner.clean(open_vidname)
        except:
    ...
    if video_type == 'movie':
        listitem.setInfo(type="Video", infoLabels={'title': video['name'], 'year': vid_year, 'type': 'movie',
                                                   'plotoutline': plot_outline, 'plot': vid_plot, 'mpaa': mpaa})
    ...
    if video_type == 'episode':
        ...
        listitem.setInfo('video',
                         {'title': video['name'], 'tvshowtitle': show['name'], 'year': vid_year, 'episode': episode_num,
                          'season': episode_season, 'type': 'episode', 'plotoutline': plot_outline, 'plot': vid_plot,
                          'mpaa': mpaa})
            ...
            if addon.get_setting('realdebrid_token'):
                ...
                if not link:
                    Notify('big', 'Real-Debrid', 'Error occurred attempting to stream the file.', '', '',
                           line3='**Attempting to resolve with original host instead..')
                    link = None
                else:
...
def GetSource():
    ...
    body = GetURL(ICEFILMS_AJAX % (id, t), params=params, referrer=ICEFILMS_AJAX_REFER % t, use_cookie=True,
                  use_cache=False)
...
def Stream_Source(name, download_play=False, download=False, download_jdownloader=False, stacked=False):
    ...
    addon.log_debug(
        'Entering Stream Source with options - Name: %s Url: %s DownloadPlay: %s Download: %s Stacked: %s' % (
        name, url, download_play, download, stacked))
...
def play_with_watched(url, listitem, mypath, last_part=False, resume_point=0, resume_threshhold=1):
    ...
    mplayer = MyPlayer(axelhelper=axelhelper, download_id=download_id, ice_url=video_url, imdbid=imdbnum,
                       season=season_num, episode=episode_num, resume_point=resume_point,
                       resume_threshhold=resume_threshhold, enableRecent=enable_recent)
...
class MyPlayer(xbmc.Player):
    def __init__(self, axelhelper=None, download_id=None, ice_url=None, imdbid=None, season=None, episode=None,
                 resume_point=0, resume_threshhold=1, enableRecent=False):
        self.dialog = None
    ...
    def onPlayBackEnded(self):
        ...
        if finalPart:
            try:
                ...
                percentWatched = 0
            addon.log_debug(
                'current time: ' + str(currentTime) + ' total time: ' + str(totalTime) + ' percent watched: ' + str(
                    percentWatched))
            vidname = cache.get('videoname')
            video = get_video_name(vidname)
    ...
    def onPlayBackStopped(self):
        ...
        if finalPart:
            try:
                ...
                percentWatched = 0
            addon.log_debug('Playback stopped - current time: ' + str(currentTime) + ' total time: ' + str(
                totalTime) + ' percent watched: ' + str(percentWatched))
            vidname = cache.get('videoname')
            video = get_video_name(vidname)
    ...
    def setRecentWatched(self, video):
        if self.enableRecent:
            ...
            db_connection.set_watched(self.ice_url, video_type, video['name'], video['year'], self.season, self.episode,
                                      self.imdbid)
...
def addExecute(name, args, mode, ice_meta, stacked=False, video_url=None):
    ...
    u = sys.argv[0] + "?url=" + sysurl + "&mode=" + str(mode) + "&name=" + sysname + "&imdbnum=" + urllib.quote_plus(
        str(imdbnum)) + "&videoType=" + str(video_type) + "&season=" + str(season_num) + "&episode=" + str(
        episode_num) + "&stackedParts=" + str(stacked) + "&" + str(argsenc) + '&videoUrl=' + urllib.quote_plus(
        video_url)
    ok = True

    liz = xbmcgui.ListItem(name, iconImage="DefaultVideo.png", thumbnailImage=ice_meta['poster'])
    liz.setInfo(type="Video", infoLabels={"Title": name, 'year': ice_meta['year'], 'type': 'movie',
                                          'plotoutline': ice_meta['plot_outline'], 'plot': ice_meta['plot'],
                                          'mpaa': ice_meta['mpaa']})
    ...
    contextMenuItems.append(('Play Stream', 'XBMC.RunPlugin(%s?mode=200&name=%s&url=%s&stackedParts=%s&%s)' % (
    sys.argv[0], sysname, sysurl, stacked, argsenc)))
    contextMenuItems.append(('Download', 'XBMC.RunPlugin(%s?mode=201&name=%s&url=%s&stackedParts=%s&%s)' % (
    sys.argv[0], sysname, sysurl, stacked, argsenc)))
    contextMenuItems.append(('Download And Watch', 'XBMC.RunPlugin(%s?mode=206&name=%s&url=%s&stackedParts=%s&%s)' % (
    sys.argv[0], sysname, sysurl, stacked, argsenc)))
    contextMenuItems.append(('Download with jDownloader',
                             'XBMC.RunPlugin(%s?mode=202&name=%s&url=%s&stackedParts=%s&%s)' % (
                             sys.argv[0], sysname, sysurl, stacked, argsenc)))
...
def addDir(name, url, mode, iconimage, meta=False, imdb=False, delfromfav=False, disablefav=False, searchMode=False,
           totalItems=0, disablewatch=False, meta_install=False, favourite=False, recentWatched=False, queueList=False):
    ...
    if meta == False:
        ...
        if mode == 12:  # TV series
            ...
            if favourite:
                next_aired = str2bool(addon.get_setting('next-aired'))
                if next_aired:
                    contextMenuItems.append(('Show Next Aired', 'RunScript(%s)' % os.path.join(icepath,
                                                                                               'resources/script.tv.show.next.aired/default.py')))
        elif mode == 13:  # TV Season
            addWatched = True
            # if tv_fanart == 'true' and tv_fanart_installed == 'true':
            if tv_fanart == 'true':
                liz.setProperty('fanart_image', meta['backdrop_url'])
            season = meta['season']
            contextMenuItems.append(('Refresh Info',
                                     'XBMC.RunPlugin(%s?mode=998&name=%s&url=%s&imdbnum=%s&dirmode=%s&videoType=%s&season=%s)' % (
                                     sys.argv[0], sysname, sysurl, urllib.quote_plus(str(imdb)), dirmode, videoType,
                                     season)))
        elif mode == 14:  # TV Episode
            addWatched = True
            if tv_fanart == 'true':
            ...
            if not queueList and not recentWatched:
                contextMenuItems.append(('Add to Queue List',
                                         'XBMC.RunPlugin(%s?mode=add_queue&name=%s&url=%s&imdbnum=%s&dirmode=%s&videoType=%s&season=%s&episode=%s)' % (
                                         sys.argv[0], sysname, sysurl, urllib.quote_plus(str(imdb)), dirmode, videoType,
                                         season, episode)))

            contextMenuItems.append(('Episode Information', 'XBMC.Action(Info)'))
            contextMenuItems.append(('Refresh Info',
                                     'XBMC.RunPlugin(%s?mode=997&name=%s&url=%s&imdbnum=%s&dirmode=%s&videoType=%s&season=%s&episode=%s)' % (
                                     sys.argv[0], sysname, sysurl, urllib.quote_plus(str(imdb)), dirmode, videoType,
                                     season, episode)))
        elif mode == 100:  # movies
            addWatched = True
            ...
            if not queueList and not recentWatched:
                contextMenuItems.append(('Add to Queue List',
                                         'XBMC.RunPlugin(%s?mode=add_queue&name=%s&url=%s&imdbnum=%s&dirmode=%s&videoType=%s)' % (
                                         sys.argv[0], sysname, sysurl, urllib.quote_plus(str(imdb)), dirmode,
                                         videoType)))

            contextMenuItems.append(('Movie Information', 'XBMC.Action(Info)'))
            contextMenuItems.append(('Search for Similar',
                                     'XBMC.RunPlugin(%s?mode=991&name=%s&url=%s&tmdbnum=%s&dirmode=%s&videoType=%s)' % (
                                     sys.argv[0], sysname, sysurl, urllib.quote_plus(str(meta['tmdb_id'])), dirmode,
                                     videoType)))
        # Add Refresh & Trailer Search context menu
        if searchMode == False:
            if mode in (12, 100):
                contextMenuItems.append(('Refresh Info',
                                         'XBMC.RunPlugin(%s?mode=999&name=%s&url=%s&imdbnum=%s&dirmode=%s&videoType=%s)' % (
                                         sys.argv[0], sysname, sysurl, urllib.quote_plus(str(imdb)), dirmode,
                                         videoType)))
        ...
        if addWatched and not disablewatch:
            ...
            if searchMode == False:
                contextMenuItems.append((watchedMenu,
                                         'XBMC.RunPlugin(%s?mode=990&name=%s&url=%s&imdbnum=%s&videoType=%s&season=%s&episode=%s)'
                                         % (
                                         sys.argv[0], sysname, sysurl, urllib.quote_plus(str(imdb)), videoType, season,
                                         episode)))
    ...
    if disablefav is False:  # disable fav is necessary for the scrapes in the homepage category.
        if delfromfav is True:
            ...
            contextMenuItems.append(('Delete from Ice Favourites',
                                     'XBMC.RunPlugin(%s?mode=111&name=%s&url=%s)' % (sys.argv[0], sysname, sysurl)))
        else:
            ...
            if mode == 100 or mode == 12:
                ...
                # if searchMode==False:
                contextMenuItems.append(('Add to Ice Favourites',
                                         'XBMC.RunPlugin(%s?mode=110&name=%s&url=%s&imdbnum=%s&videoType=%s)' % (
                                         sys.argv[0], sysname, sysurl, sysimdb, videoType)))

    if recentWatched:
        contextMenuItems.append(('Delete from Watched List', 'XBMC.RunPlugin(%s?mode=remove_watched&name=%s&url=%s)' % (
        sys.argv[0], sysname, sysurl)))

    if queueList:
        contextMenuItems.append(('Delete from Queue List', 'XBMC.RunPlugin(%s?mode=remove_queue&name=%s&url=%s)' % (
        sys.argv[0], sysname, sysurl)))
    ...
    if mode in (12, 13, 100, 101):
        u = sys.argv[0] + "?url=" + sysurl + "&mode=" + str(
            mode) + "&name=" + sysname + "&imdbnum=" + urllib.quote_plus(
            str(imdb)) + "&videoType=" + videoType + "&season=" + str(season) + "&episode=" + str(episode)
    else:
...
def cleanUnicode(string):
    try:
        string = string.replace("'", "").replace(unicode(u'\u201c'), '"').replace(unicode(u'\u201d'), '"').replace(
            unicode(u'\u2019'), '').replace(unicode(u'\u2026'), '...').replace(unicode(u'\u2018'), '').replace(
            unicode(u'\u2013'), '-')
        return string
    except:
        return string
...
def get_episode(season, episode, imdb_id, url, metaget, meta_installed, tmp_season_num=-1, tmp_episode_num=-1,
                totalitems=0):
    # displays all episodes in the source it is passed.
...
def SearchForTrailer(search, imdb_id, type, manual=False):
    ...
    if manual:
        ...
        for res in results:
            if res.url.encode('utf8').startswith('http://www.youtube.com/watch') and res.url.encode(
                    'utf8') not in res_url:
                res_name.append(res.title.encode('utf8'))
                res_url.append(res.url.encode('utf8'))
...
def SimilarMovies(tmdb_id):
    ...
    if movie_list:
        ...
        if index > -1:
            xbmc.executebuiltin("XBMC.Container.Update(%s?mode=555&url=%s&search=%s&nextPage=0)" % (
            sys.argv[0], iceurl, name_list[index]))
...
def addDownloadControls(name, localFilePath, listitem=None):
    ...
    # add Download info
    ok = xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=statusUrl, listitem=xbmcgui.ListItem("Download Info"),
                                     isFolder=False)
    addon.log_debug('Ok: %s' % ok)

    # add Cancel Download
    ok = ok and xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]), url=cancelUrl,
                                            listitem=xbmcgui.ListItem("Cancel Download"), isFolder=False)
    addon.log_debug('Ok: %s' % ok)
...
def show_addon_help():
    ...
    textBox.setText('[B]Author:[/B] %s\n[B]Current version:[/B] %s\n[B]Support:[/B] www.tvaddons.ag.com' % (
    addon.get_author(), addon.get_version()))

    # Installed dependencies
    textBox = pyxbmct.TextBox(textColor='0xFFFFFFFF')
    window.placeControl(textBox, 3, 0, columnspan=7, rowspan=3)
    textBox.setText(
        '[B]Installed Dependencies:[/B]\n    [B]Metahandlers:[/B] %s \n    [B]Common addon methods:[/B] %s \n    [B]Axel Downloader:[/B] %s' % (
        metahandler_version, common_addon_version, axel_addon_version))
...
if mode == 'main':  # or url==None or len(url)<1:
    ...
    addon.log_debug(
        "Mode 998 (season meta refresh) ******* dirmode is " + str(dirmode) + " *************  url is -> " + url)
    season_refresh(url, imdbnum, name, dirmode, season_num)

elif mode == '997':
    addon.log_debug(
        "Mode 997 (episode meta refresh) ******* dirmode is " + str(dirmode) + " *************  url is -> " + url)
    ...
    addon.log_debug("Mode 990 (Change watched value) ******* name is " + str(
        name) + " *************  season is -> '" + season_num + "'" + " *************  episode is -> '" + episode_num + "'")
...
# Some tv shows will not be correctly identified, so to load their sources need to check on mode==14
    ...
    addon.log_debug(
        "Mode 555 (Get More...) ******* search string is " + search + " *************  nextPage is " + nextPage)
